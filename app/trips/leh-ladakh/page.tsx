...
export default function LehLadakhPage() {
  ...
  const [related2CarouselIndex, setRelated2CarouselIndex] = useState(0)
  ...
  // Get related2 packages
  const related2Packages = useMemo(() => {
    return getTripsBySection('related2')
  }, [])
                ...
                {isMobile ? (
                  ...
                  <>
                    ...
                    <div className="relative">
                      ...
                      <button
                        onClick={() => {
                          setRelated2CarouselIndex((prev) => Math.max(prev - 1, 0))
                        }}
                        disabled={related2CarouselIndex === 0}
                        className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white/80 backdrop-blur-md w-10 h-10 rounded-full flex items-center justify-center shadow-md hover:scale-110 transition disabled:opacity-40"
                      >
                        <ChevronLeft size={20} />
                      </button>
                      ...
                      <button
                        onClick={() => {
                          const maxIndex = Math.max(0, related2Packages.length - cardsPerView)
                          setRelated2CarouselIndex((prev) => Math.min(prev + 1, maxIndex))
                        }}
                        disabled={related2CarouselIndex === Math.max(0, related2Packages.length - cardsPerView)}
                        className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white/80 backdrop-blur-md w-10 h-10 rounded-full flex items-center justify-center shadow-md hover:scale-110 transition disabled:opacity-40"
                      >
                        <ChevronRight size={20} />
                      </button>
                      ...
                      <div className="overflow-hidden">
                        <div
                          className="flex transition-transform duration-500 ease-in-out"
                          style={{
                            transform: `translateX(-${related2CarouselIndex * (100 / cardsPerView)}%)`,
                          }}
                        >
                          {related2Packages.map((trip) => (
                            <div
